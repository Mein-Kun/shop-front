...
export const updateOrderShoppingCart = shoppingCart.createEvent<{
  userId: number
  order: number
}>()
...
function updateAdminCartItem<T>(
  cartItems: IShoppingCartItem[],
  userId: number,
  payload: T
) {
...
export const $shoppingCart = shoppingCart
  .createStore<IShoppingCartItem[]>([] as IShoppingCartItem[])
  .on(setShoppingCart, (_, shoppingCart) => shoppingCart)
  .on(updateShoppingCart, (state, cartItem) => [...state, cartItem])
  .on(updateOrderShoppingCart, (state, { order, userId }) => [
    ...updateAdminCartItem(state, userId, { order }),
  ])
